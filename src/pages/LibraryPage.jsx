...
export function LibraryPage() {
  ...
  if (!quizzes.data?.length) return (
    <div className="page stack gap-4"><header><h1 className="page-title">My quizzes</h1></header><EmptyState icon={BookOpen} title="Your library is empty" description="Create a quiz to see it here." action={<Link to="/quiz"><Button>Create a quiz</Button></Link>} /></div>
  );

  return (
    <div className="page stack gap-4">
      <header><h1 className="page-title">My quizzes</h1></header>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {quizzes.data.map((quiz) => {
          ...
          return (
            <Card key={quiz.id} className="stack gap-3">
              <div><span className="eyebrow">{session?.levelTitle || "Ready to begin"}</span><h2 className="mb-1 mt-1 text-lg font-black">{quiz.title}</h2><p className="muted m-0 line-clamp-2 text-sm leading-6">{quiz.description}</p></div>
