...
const statusDetails = {
  valid: { label: "صالح", className: "bg-[var(--color-brand-soft)] text-brand", icon: CheckCircle2 },
  invalid: { label: "غير صالح", className: "bg-red-50 text-[var(--color-danger)]", icon: CircleAlert },
  untested: { label: "غير مختبر", className: "bg-canvas text-muted", icon: KeyRound }
};
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold ${details.className}`}>
...
export function GroqKeySettings({ user }) {
  ...
  async function deleteKey(item) {
    if (!window.confirm(`واش متأكد بغيتي تحذف “${item.label}”؟`)) return;
    await run(`delete:${item.id}`, async () => {
      await groqKeyService.remove({ keyId: item.id });
      setMessage("تم حذف المفتاح نهائياً.");
      await loadKeys();
    });
  }
  ...
  return (
    <Card className="lg:col-span-2">
      <div className="mb-5 flex items-start gap-3">
        <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[var(--color-brand-soft)] text-brand">
          <KeyRound size={20} aria-hidden="true" />
        </span>
        <div>
          <span className="eyebrow">Groq API Keys</span>
          <h2 className="mb-1 mt-1 text-xl font-black">مفاتيح Groq الخاصة بك</h2>
          <p className="muted m-0 text-sm leading-6">تُشفّر داخل Supabase Vault. بعد الحفظ لن نعرض المفتاح الكامل مرة أخرى.</p>
        </div>
      </div>

      {!user ? (
        <div className="rounded-xl border border-line bg-canvas p-4 text-sm">
          سجّل الدخول من بطاقة المزامنة باش تضيف وتدير مفاتيحك.
        </div>
      ) : (
          ...
          <form className="stack content-start" onSubmit={saveKey}>
            ...
            <label className="stack gap-2 text-sm font-bold">
              ...
              <span className="relative">
                <input
                  dir="ltr"
                  className="min-h-12 w-full rounded-xl border border-line bg-canvas py-3 pl-12 pr-4 outline-none focus:border-brand"
                ...
                <button
                  type="button"
                  className="absolute inset-y-0 left-0 grid w-12 place-items-center text-muted hover:text-ink"
            ...
            <p className="muted m-0 text-xs leading-5">يمكنك حفظ حتى 10 مفاتيح. عند تعطل مفتاح، ينتقل الـQuiz تلقائياً للمفتاح التالي.</p>
          </form>

          <section aria-labelledby="saved-groq-keys">
            <div className="mb-3 flex items-center justify-between gap-3">
              <h3 id="saved-groq-keys" className="m-0 text-base font-black">المفاتيح المحفوظة</h3>
            ...
            {loading ? (
              <div className="flex min-h-28 items-center justify-center gap-2 text-sm text-muted" role="status">
                <LoaderCircle className="animate-spin" size={18} /> كنحمّل المفاتيح…
              </div>
            ) : keys.length ? (
              ...
              <div className="grid min-h-28 place-items-center rounded-xl border border-dashed border-line bg-canvas p-4 text-center text-sm text-muted">
                مازال ما حفظتي حتى مفتاح.
              </div>
            )}
          </section>
        </div>
      ...
      <div className="mt-4" aria-live="polite">
        ...
        {message ? (
          <p className="m-0 flex items-center gap-2 rounded-xl bg-[var(--color-brand-soft)] p-4 text-sm font-semibold text-brand">
