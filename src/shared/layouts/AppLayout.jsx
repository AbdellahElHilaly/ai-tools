...
function PrimaryItem({ item, pathname, onNavigate }) {
  ...
  return (
    <Link to={item.to} onClick={onNavigate} aria-current={active ? "page" : undefined} className="primary-nav-item">
...
function ContextItem({ item, location }) {
  ...
  return (
    <Link to={item.to} aria-current={active ? "page" : undefined} className="context-nav-item">
...
export function AppLayout() {
  ...
  const title = navigationTitles[section];
  ...
  return (
    <div className={`app-shell ${theme.className}`} data-module-theme={theme.id}>
      <header className="app-header">
        <strong className="app-header__module">{title}</strong>
        <button
          type="button"
      ...
      {drawerOpen ? (
        <button type="button" className="drawer-scrim" onClick={() => setDrawerOpen(false)} aria-label="Close sidebar overlay" />
      ) : null}

      <aside className={`app-sidebar ${drawerOpen ? "app-sidebar--open" : ""}`}>
        <div className="app-sidebar__brand">
          <Brand />
          <button type="button" className="icon-button" onClick={() => setDrawerOpen(false)} aria-label="Close sidebar">
            <PanelLeftClose size={20} />
          </button>
        </div>
        <nav className="primary-nav" aria-label="Primary navigation">
          {primaryNavigation.map((item) => (
            <PrimaryItem key={item.to} item={item} pathname={location.pathname} onNavigate={() => setDrawerOpen(false)} />
          ))}
        </nav>
      </aside>
